...
2. **0:20–0:45 — shared surface.** Show the Collserola–Vallvidrera pack loading on the deployed page: OSM reference map, TrailPack provenance, and the verified Vallvidrera access.
3. **0:45–1:30 — agent action.** In the ChatGPT desktop app built-in browser, ask: “Plan a 7.2 km loop from Vallvidrera access, preferring official matches.” Show the site-tools indicator, the invocation log, and the rendered loop.
...
```text
Use the Switchback site tools on this page. Plan a 7.2 km loop from
vallvidrera_access with prefer_waymarked true. Then call get_route_summary.
```
