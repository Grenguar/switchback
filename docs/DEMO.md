...
5. **2:10–2:35 — handoff.** Ask the agent to compare the next three forecast days, then prepare a family briefing. Show the visible forecast caveat, the Copy briefing control, then prepare and download GPX. State the limits plainly: official matches are evidence, forecasts are not field conditions or safety clearance, and ascent is estimated only after rendering, not used to constrain route selection.
