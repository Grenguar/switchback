...
2. **0:20–0:45 — shared surface.** Show Collserola loading on the deployed page: OSM reference map, TrailPack provenance, and the GR-6 Horta access.
3. **0:45–1:30 — agent action.** In the ChatGPT desktop app built-in browser, ask: “Plan a 7.2 km loop from GR-6 Horta access, preferring official matches.” Show the site-tools indicator, the invocation log, and the rendered loop.
...
gr6_horta_access with prefer_waymarked true. Then call get_route_summary.
```
