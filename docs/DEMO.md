...
2. **0:20–0:45 — shared surface.** Show the Collserola–Vallvidrera pack loading on the deployed page: OSM reference map, TrailPack provenance, and the Font Groga car-park trailhead.
3. **0:45–1:30 — agent action.** In the ChatGPT desktop app built-in browser, ask: “Plan a 7 km loop from Font Groga car park, preferring official matches.” Show the site-tools indicator, the invocation log, and the rendered loop.
...
font_groga_parking with prefer_waymarked true. Then call get_route_summary.
```
