...
const required = ["index.html", "trailpack/manifest.json", "trailpack/tarragona-demo.json"];
for (const file of required) await access(resolve(dist, file), constants.R_OK);

const [html, manifestText, artifactInfo] = await Promise.all([
  readFile(resolve(dist, "index.html"), "utf8"),
  readFile(resolve(dist, "trailpack/manifest.json"), "utf8"),
  stat(resolve(dist, "trailpack/tarragona-demo.json"))
]);
...
console.log(JSON.stringify({
  ...
  trailpack_artifact_bytes: artifactInfo.size,
