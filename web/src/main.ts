  ...
  <main class="shell">
    ...
    <section class="workspace" aria-label="Route planning workspace">
      <aside class="planner"><div class="section-label"><span>01</span><p>Plan a walk</p></div>
        <form id="plan-form"><div class="route-question"><label for="route-request">What sounds good?</label><div class="route-request-row"><input id="route-request" type="text" autocomplete="off" placeholder="A relaxed 7.5 km loop from Vista Rica parking" /><button class="plan-button compact" type="submit">Plan it <span aria-hidden="true">↗</span></button></div><p class="field-hint" id="route-request-hint">Or pick a car park and distance below. Nothing is sent anywhere.</p></div><fieldset class="start-picker"><legend>Park, then walk</legend><div class="start-options" role="group" aria-label="Available parking starts">${Object.values(documentedStarts).map((start, index) => `<button class="start-option" type="button" data-start="${start.id}" aria-pressed="${index === 0}"><strong>${start.name}</strong><small>${start.description}</small></button>`).join("")}</div><p class="field-hint" id="start-hint">Only parking locations with a verified connection to the trail graph are shown.</p></fieldset><fieldset class="distance-picker"><legend>How far today? <output id="distance-value">7</output> km</legend><input id="distance" name="distance" type="range" min="1" max="30" step="0.5" value="7" aria-describedby="distance-help" /><div class="distance-steps" role="group" aria-label="Distance in kilometres">${Array.from({ length: 30 }, (_, index) => `<button type="button" class="distance-step" data-distance="${index + 1}" aria-pressed="${index + 1 === 7}">${index + 1}</button>`).join("")}</div><p class="field-hint" id="distance-help">Use the slider for half-kilometres. Every result must land within ±0.5 km of your request.</p></fieldset><fieldset><legend>Route character</legend><div class="choices"><label><input type="radio" name="character" value="waymarked" checked /> Prefer official-match evidence</label><label><input type="radio" name="character" value="neutral" /> Just find me a trail loop</label></div></fieldset><p class="field-hint">The graph excludes urban footways and paved access roads. Official matching is evidence, not a waymarking confirmation. Elevation and grade constraints are not available yet.</p></form>
...
const applyPlainLanguageRequest = (): void => {
  ...
  const requestedDistance = request.match(/\b([1-9]|[12]\d|30)(?:[.,]5)?\s*(?:km|kilomet(?:re|er|ers|res)?s?)\b/i)?.[0]?.match(/[\d.,]+/)?.[0];
  if (requestedDistance) setDistance(Number(requestedDistance.replace(",", ".")));
  if (request.includes("vista rica")) setStart("vista_rica_parking");
  else if (request.includes("passeig de les aig") || request.includes("aigues")) setStart("passeig_aigues_parking");
