...
const actionLabel: Record<string, string> = {
  list_circuit_options: "Listing verified loop options", validate_circuit: "Checking a circuit before drawing it", record_session_note: "Writing a visible session note", plan_route: "Planning and drawing the loop", get_route_summary: "Reading the active route", explain_difficulty: "Reviewing route difficulty evidence", explain_segment: "Inspecting a trail segment", avoid_segment: "Replanning around a segment", prepare_gpx: "Preparing the GPX handoff", get_trail_weather: "Comparing the next three forecast days", get_hiking_conditions: "Assembling hiking decision context", get_park_alerts: "Checking official Park alerts", prepare_route_briefing: "Preparing the group briefing", describe_last_edit: "Reviewing the latest map edit", webmcp_registration: "Checking the WebMCP connection",
};
...
const weatherEmoji = (summary: string): string => summary === "clear" ? "☀️" : summary === "cloudy" ? "☁️" : summary === "fog" ? "🌫️" : summary === "drizzle" ? "🌦️" : summary === "rain" || summary === "showers" ? "🌧️" : summary === "thunderstorms" ? "⛈️" : "🌤️";
const forecastText = (window: WeatherWindow): string => `${window.start}–${window.end} · ${window.summary} · ${window.temperatureC}°C · ${window.precipitationProbability}% rain · gusts ${window.gustKph} km/h`;
...
const renderTrailWeather = (forecast: TrailWeather): void => {
  ...
  if (best) best.textContent = `Best overall: ${weatherEmoji(forecast.bestWindow.summary)} ${forecast.bestWindow.date} · ${forecastText(forecast.bestWindow)} (${forecast.timezone}).`;
  if (days) {
    days.replaceChildren(...[...bestByDay.values()].sort((left, right) => left.date.localeCompare(right.date)).map((window) => {
      const evening = forecast.windows.find((candidate) => candidate.date === window.date && candidate.start === "17:00");
      const item = document.createElement("li"); item.className = "weather-day";
      const heading = document.createElement("strong"); const details = document.createElement("span"); const daylight = document.createElement("small");
      heading.textContent = `${weatherEmoji(window.summary)} ${window.daylight?.weekday ?? window.date} · ${window.date}`;
      details.textContent = `Best: ${forecastText(window)}${evening ? ` · Evening: ${weatherEmoji(evening.summary)} ${forecastText(evening)}` : ""}`;
      daylight.textContent = window.daylight?.sunrise && window.daylight?.sunset ? `🌅 sunrise ${window.daylight.sunrise} · 🌇 sunset ${window.daylight.sunset}${evening?.crossesSunset ? " · evening window crosses sunset" : ""}` : "Sunrise and sunset unavailable for this forecast.";
      item.append(heading, details, daylight); return item;
    }));
  }
  if (status) status.textContent = "Next 3 days. Forecast and daylight are planning context only; check official alerts, closures, and local conditions.";
  ...
  if (list) {
    list.replaceChildren(...alerts.alerts.slice(0, 3).map((alert) => {
      ...
      if (alert.translation) {
        ...
        translation.className = "park-alert-translation"; label.textContent = "Machine translation"; translatedTitle.textContent = alert.translation.title; translatedExcerpt.textContent = alert.translation.excerpt;
