...
const planInput = (): Record<string, unknown> => ({ start: document.querySelector<HTMLSelectElement>("#start")?.value ?? "gr6_horta_access", target_km: Number(document.querySelector<HTMLInputElement>("#distance")?.value ?? 7.2), prefer_waymarked: document.querySelector<HTMLInputElement>("input[name=character]:checked")?.value === "waymarked" });
...
const agentTestPrompt = "Use the site tools on this Switchback page. Call plan_route with start gr6_horta_access, target_km 7.2, and prefer_waymarked true. Then call get_route_summary and state the data limitations.";
