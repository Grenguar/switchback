...
export const documentedStarts = {
  ...
  panta_vallvidrera: { id: "panta_vallvidrera", name: "Pantà de Vallvidrera", latitude: 41.41520, longitude: 2.09744, availability: "available", transportMode: "car", circuitStatus: "verified", recommendedKm: 3.5, circuitDistancesKm: [2, 3.5], description: "Roadside parking near the reservoir and GR-92." },
  ...
  can_jane_sant_cugat: { id: "can_jane_sant_cugat", name: "Can Jané, Sant Cugat", latitude: 41.44457, longitude: 2.11759, availability: "available", transportMode: "car", circuitStatus: "verified", recommendedKm: 7, circuitDistancesKm: [2, 7, 14], description: "Car access for Sant Medir, with the trail network starting near Can Jané." },
  can_coll_cerdanyola: { id: "can_coll_cerdanyola", name: "Can Coll, Cerdanyola", latitude: 41.47369, longitude: 2.12437, availability: "available", transportMode: "car", circuitStatus: "verified", recommendedKm: 5, circuitDistancesKm: [2, 5, 14], description: "Quiet northern-sector car start at Can Coll." },
  ...
  mirador_xiprers: { id: "mirador_xiprers", name: "Mirador dels Xiprers", latitude: 41.38992, longitude: 2.10010, availability: "available", transportMode: "public_transport", circuitStatus: "verified", recommendedKm: 7, circuitDistancesKm: [2, 5, 7], description: "Bus 22 or Av. Tibidabo access for the Carretera de les Aigües balcony route." },
 ...
 * The TrailPack does not yet contain complete elevation or technical-grade data.
 */
...
export class TrailPlanner {
  ...
  plan(startId: StartId, targetKm: number, preferWaymarked: boolean): PlannedRoute {
    ...
    if (!winner) {
      ...
      throw new Error(`No non-retracing circuit within ±${TARGET_TOLERANCE_METRES / 1000} km of ${targetKm} km can be formed from this TrailPack start. ${diagnostic.result.message} Rejected candidates: ${diagnostic.rejected.closure} closure, ${diagnostic.rejected.distance} distance, ${diagnostic.rejected.reuse} reuse.`);
    }
