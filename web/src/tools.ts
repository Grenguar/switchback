...
const rawToolContracts: ToolContract[] = [
  {
    name: "plan_route", description: "Plan and render a real non-retracing Collserola trail circuit from a car park: `vista_rica_parking` or `passeig_aigues_parking`. The pack excludes urban footways and paved access roads. Elevation and grade filters explicitly reject because this artifact has no elevation data.", annotations: untrusted,
    inputSchema: { type: "object", additionalProperties: false, required: ["start", "target_km", "prefer_waymarked"], properties: { start: { type: "string", enum: Object.keys(documentedStarts), description: "Car-park start: Vista Rica parking or Passeig de les Aigües parking." }, target_km: { type: "number", minimum: 1, maximum: 30, multipleOf: 0.5, description: "Desired circuit distance in 0.5 km steps, from 1 through 30. A result is accepted only within ±0.5 km." }, prefer_waymarked: { type: "boolean", description: "Bias toward the Park’s published A–E marked-path network while retaining OSM trail connectors needed to close a loop. It does not confirm present-day conditions." }, max_ascent_m: { type: "number", description: "Unsupported: this TrailPack has no elevation values, so the request is rejected." }, max_grade: { type: "string", description: "Unsupported: this TrailPack has incomplete grade tags, so the request is rejected." } } },
