...
const routeOutput = (route: PlannedRoute) => ({ route: route.name, start: route.start.name, arrival_mode: route.start.transportMode, distance_km: route.distanceKm, ascent_m: null, duration_hours: route.durationHours, returns_to_start: true, shared_access_percent: route.sharedAccessPercent, official_match_percent: route.waymarkedPercent, source: route.source, attribution: provenance });
...
const rawToolContracts: ToolContract[] = [
  {
    name: "plan_route", description: "Plan and render a genuine Collserola circuit from a car or public-transport origin. It returns to its graph start and rejects long out-and-back retracing. Use an origin with verified circuit status; other published access points are preserved for future point-to-point planning. The pack excludes urban footways and paved access roads. Elevation and grade filters explicitly reject because this artifact has no elevation data.", annotations: untrusted,
    inputSchema: { type: "object", additionalProperties: false, required: ["start", "target_km", "prefer_waymarked"], properties: { start: { type: "string", enum: Object.keys(documentedStarts), description: "Named Collserola origin. Inspect arrival_mode and choose a verified circuit origin." }, arrival_mode: { type: "string", enum: ["car", "public_transport"], description: "Optional arrival context. When supplied, it must agree with the chosen origin." }, target_km: { type: "number", minimum: 1, maximum: 30, multipleOf: 0.5, description: "Desired circuit distance in 0.5 km steps, from 1 through 30. A result is accepted only within ±0.5 km." }, prefer_waymarked: { type: "boolean", description: "Bias toward the Park’s published A–E marked-path network while retaining OSM trail connectors needed to close a loop. It does not confirm present-day conditions." }, max_ascent_m: { type: "number", description: "Unsupported: this TrailPack has no elevation values, so the request is rejected." }, max_grade: { type: "string", description: "Unsupported: this TrailPack has incomplete grade tags, so the request is rejected." } } },
    execute: async (input, signal) => {
      ...
      const start = choice(data.start, "start", Object.keys(documentedStarts) as StartId[]); const targetKm = halfKilometres(data.target_km, "target_km");
