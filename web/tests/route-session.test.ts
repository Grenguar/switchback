...
const artifact: TrailPackArtifact = {
  manifest,
  tiles: { demo: { nodes: [point(41.3081881, 0.9670976), point(41.309, 0.972), point(41.3075, 0.975), point(41.306, 0.968)], edges: [
...
test("replanViaWaypoint returns a bounded directed non-retracing loop", () => {
  ...
  const original = planner.plan("gr65_access", 3, true);
  const replanned = planner.replanViaWaypoint(original, { latitude: 41.306, longitude: 0.968 }, true);
...
test("RouteSession commits waypoint route and measured deltas atomically", () => {
  const planner = new TrailPlanner(artifact);
  const initial = planner.plan("gr65_access", 3, true);
  const session = new RouteSession(initial, 3);

  const edit = session.commitWaypoint(planner, { latitude: 41.306, longitude: 0.968 }, true);
